# Build in cloud — Internal Teams agent using MCP tools against an approved ser
...
### 1. Create agent
- Create → New agent → Name: `Internal Teams agent using MCP tools against an approved ser`.
...
- Configure: MCP server tools (generative orchestration required; classify MCP connector in DLP)
